...
def delete_source_entries_on_disk(source_db, data_dir):
    """
    Delete on disk all submissions and replies associated
    with a given source.
    """
    files_to_delete = os.path.join(
        data_dir,
        '*{}*'.format(source_db.journalist_designation))
    logging.info('Deleting all {} files'.format(files_to_delete))
    delete_all_matching_files_in_directory(files_to_delete)
...
def delete_all_matching_files_in_directory(file_matching_string):
    """
    Delete all files that match a given string.
    """
    try:
        for file_to_delete in glob.glob(file_matching_string):
